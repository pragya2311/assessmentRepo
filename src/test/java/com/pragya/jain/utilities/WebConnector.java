...
public class WebConnector {
	...
	public WebConnector() {
		...
		if(prop==null) {
			...
			try {
				prop= new Properties();
				FileInputStream fs = new FileInputStream(System.getProperty("user.dir")+"\\src\\test\\resources\\properties\\project.properties");
				prop.load(fs);
			} catch (Exception e) {
				e.printStackTrace();
				// report
	...
	public void openBrowser(String browserName) {
		if(prop.getProperty("gridRun").equals("Y")) {
			...
			try {
				driver = new RemoteWebDriver(new URL(prop.getProperty("loginURL")),cap) {
				};
